...
def MyOpenResFile(path):
	mode = 1  # read only
	try:
		resref = Res.FSOpenResourceFile(path, u'rsrc', mode)
	except Res.Error:
		# try data fork
...
class SFNTResourceWriter:
	...
	def __init__(self, path, ttFont, res_id=None):
		...
		MacOS.SetCreatorAndType(path, 'DMOV', 'FFIL')
		self.resref = Res.FSOpenResourceFile(self.name, u'rsrc', 3)  # exclusive read/write permission
